...
namespace Jpeg2000Library.Util
{
     ...
     * @see Properties
     * */
    public class ParameterList : Properties
    {
        ...
        public void parseArgs(String[] argv)
        {
            ...
            if (c != '-' && c != '+')
            { // It's not an option
                throw new FormatException("Argument list does not" +
                                                " start with an option: " +
                                                argv[k]);
            }
            if (argv[k].Length >= 2 &&
                char.IsDigit(argv[k][1]))
            {
                throw new FormatException("Numeric option name: " + argv[k]);
            }
            ...
            while (k < argv.Length)
            {
                ...
                if (argv[k].Length <= 1)
                {
                    throw new FormatException("Option \"" + argv[k] +
                                                    "\" is too short.");
                }
                c = argv[k][0];
                pname = argv[k++];
                ...
                else
                {
                    ...
                    { // Next word could be an option
                        ...
                        {
                            throw
                                new FormatException("Option or argument \""
                                                          + argv[k] +
                                                          "\" too short");
                        }
                    ...
                    { // No value yet
                      // It should not a boolean option, read the values
                        if (c == '+')
                        {
                            throw new FormatException("Boolean option \"" +
                                                            pname +
                                                            "\" has a value");
                        }
                        ...
                        while (k < argv.Length)
                        {
                            ...
                            if (c == '-' || c == '+')
                            {
                                ...
                                if (argv[k].Length <= 1)
                                {
                                    throw new FormatException("Option or " +
                                                              "argument \""
                                                              + argv[k] +
                                                              "\" too short");
                                }
                ...
                if (get(pname.Substring(1)) != null)
                {
                    ...
                    throw new FormatException("Option \"" + pname +
         ...
         * */
        public bool getBooleanParameter(String pname)
        {
            String s = (String)getParameter(pname);

            if (s == null)
            {
                throw new ArgumentException("No parameter with name " +
                                                   pname);
            }
            else if (s == ("on"))
            {
            ...
            {
                throw new FormatException("Parameter \"" + pname +
                                                "\" is not boolean: " + s);
            }
        }
